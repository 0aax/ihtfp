...
def main():
    ...
    if len(args) == 0 or args[0] == 'daily':
        ...
        if len(args) != 3:
            print(format_error('Invalid number of parameters.'))
            sys.exit(0)
        meaning = args[1]
        rating = args[2]
        if not rating.isdigit() or int(rating) > 10 or int(rating) == 0:
            print(format_error('Invalid mood rating.'))
            sys.exit(0)
        rating = int(rating)
        ...
        if len(args) != 2:
            print(format_error('Incorrect number of parameters.'))
            sys.exit(0)
        meaning = args[1]
        deleted = 0
        ...
        if args[1] == 'color':
            ...
            try: new = [int(c) for c in color]
                ...
                print(format_error('Invalid rgb value.'))
                sys.exit(0)
            cfg['color'] = new
            new = tuple(new)
            name = 'color'
            ...
            time = args[2]
            if not time.isdigit():
                print(format_error('Invalid time period.'))
                sys.exit(0)
            new = int(time)
            cfg['time'] = new
            name = 'time'
            ...
            nops = args[2]
            if not nops.isdigit():
                print(format_error('Invalid number of options.'))
                sys.exit(0)
            new = int(nops)
            cfg['num_options'] = new
            name = 'number of options'
        else:
            print(format_error('{} is not a valid config variable'.format(args[1])))
            sys.exit(0)
